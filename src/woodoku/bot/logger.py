...
def log_turn(turn: int, board_before, piece_ids, moves, shape_sizes) -> None:
    ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    lines = [f"\n{'=' * 52}", f"TURN {turn}  [{ts}]", f"{'=' * 52}", f"Pieces:  {piece_ids}", "Board before:", _board_str(board_before), "Moves:"]
    for i, (shape_id, row, col) in enumerate(moves, 1):
        h, w = shape_sizes.get(shape_id, (1, 1))
        lines.append(f"  {i}. shape {shape_id:3d} ({h}x{w})  ->  row={row}, col={col}")
    _logger.info("\n".join(lines))
